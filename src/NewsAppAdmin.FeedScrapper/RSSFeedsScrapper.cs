...
namespace NewsAppAdmin.FeedScrapper
{
    public class RSSFeedsScrapper
    {
        ...
        public static void GetRSSFeeds(string feedsUrl)
        {
           ModelFeedDetail modelfeedDetail = new ModelFeedDetail();
            try
            {
                ...
                if (xd != null)
                {
                    XmlNodeList lstNodes = xd.SelectNodes("rss/channel/item");
                    if (lstNodes != null)
                    {
                        foreach (XmlNode node in lstNodes)
                        {
                            //Get Title
                            XmlNode getTile = node.SelectSingleNode("title");
                            if (getTile != null && !String.IsNullOrEmpty(getTile.InnerText.Trim()))
                            {
                                modelfeedDetail.PostTitle = getTile.InnerText.Trim();
                            }
                            else
                            {
                                modelfeedDetail.PostTitle= String.Empty;
                            }

                            //Get Short Desc
                            XmlNode getDescription = node.SelectSingleNode("description");
                            if (getDescription != null && !String.IsNullOrEmpty(getDescription.InnerText.Trim()))
                            {
                                modelfeedDetail.ShortDescription = getDescription.InnerText.Trim();
                            }
                            else
                            {
                                modelfeedDetail.ShortDescription = String.Empty;
                            }

                            //Get detail page link
                            XmlNode getDetailPageURL = node.SelectSingleNode("link");
                            if (getDetailPageURL != null && !String.IsNullOrEmpty(getDetailPageURL.InnerText.Trim()))
                            {
                                modelfeedDetail.DetailPageURL = getDetailPageURL.InnerText.Trim();
                            }
                            else
                            {
                                modelfeedDetail.DetailPageURL = String.Empty;
                            }

                            //Get Item Publish Date
                            XmlNode getItemPublishDate = node.SelectSingleNode("pubDate");
                            if (getItemPublishDate != null && !String.IsNullOrEmpty(getItemPublishDate.InnerText.Trim()))
                            {
                                modelfeedDetail.PublishDate = getItemPublishDate.InnerText.Trim();
                            }
                            else
                            {
                                modelfeedDetail.PublishDate = String.Empty;
                            }

                            //Get Contents
                            XmlNode getContents = node.SelectSingleNode("enclosure");
                            if (getContents != null)
                            {
                                modelfeedDetail.RssImage = getContents.Attributes.GetNamedItem("url").InnerText;
                                modelfeedDetail.Category = getContents.Attributes.GetNamedItem("type").InnerText;
                            }
                            else
                            {
                                modelfeedDetail.RssImage = String.Empty;
                                modelfeedDetail.Category = String.Empty;
                            }
